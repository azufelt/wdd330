...
function hikeCommentList(hike) {
const hikeComments = commentJSON.filter(commentarray => commentarray.name === hike);
hikeComments.forEach(comment => {
   if(comment !== '') {
      ...
      commentli.classList.add('userComment');
      commentli.innerHTML = `<p><em>${comment.date}</em></p>
      <p>${newcomment}</p>`;
    //  li.append(newcomment);s
    //  document.querySelector('#testoutput').innerHTML = `<li> ${newcomment}</li>`;
      // return newcomment;
        console.log(comment);
        return commentli
      }
      ...
      // let hikecomment = comment.content[0];
      // return hikecomment;
...
function renderOneHikeLight(hike) {
  ...
  let hikeComment = hikeCommentList(hike.name);
